...
public class Update {
    ...
    public static void main(String[] args) {
        ...
        try (MongoClient mongoClient = MongoClients.create(System.getProperty("mongodb.uri"))) {
            ...
            Document document = gradesCollection.find(filter).first();
            if (document != null) {
            ...
            System.out.println(updateResult);
            Document document2 = gradesCollection.find(filter).first();
            if (document2 != null) {
                System.out.println(document2.toJson(prettyPrint));
            } else {
                System.out.println("No document found matching the query.");
            }
            ...
            Document document3 = gradesCollection.find(filter).first();
            if (document3 != null) {
                System.out.println(document3.toJson(prettyPrint));
            } else {
                System.out.println("No document found matching the query.");
            }
            ...
            Document document4 = gradesCollection.find(filter).first(); // Retrieve the first matching document
            if (document4 != null) {
                String jsonOutput = document4.toJson(); // Convert the document to JSON
                System.out.println(jsonOutput); // Print the JSON representation
            } else {
                System.out.println("No document found matching the query.");
            }
        }
